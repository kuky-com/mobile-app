...
const EmailVerificationScreen = ({ navigation, route }) => {
  ...
  const onContinue = () => {
    ...
    setLoading(true);
    apiClient
      .post("auth/verify", { email, code: pin, device_id: deviceId, platform: Platform.OS })
      .then((res) => {
        setLoading(false);
    ...
    <View style={styles.container}>
      ...
      >
        ...
        >
          ...
          >
            <Text style={{ fontSize: 24, color: "black", fontWeight: "bold", width: "100%" }}>
              Enter your verification code
            </Text>
            <Text style={{ color: "#726E70", fontSize: 15, width: "100%", marginBottom: 10 }}>
              {`We sent a code by text to `}
              <Text style={{ color: "#5E30C1" }}>{email}</Text>
            </Text>
            <PinInput text={pin} handleTextChange={onPinUpdate} />
            <View
            ...
            <View style={{ flexDirection: "row" }}>
              <View style={{ flex: 1, gap: 16 }}>
                <Text style={{ color: "#726E70", fontSize: 15, width: "100%", flex: 1 }}>
                  {`Did't get a code? `}
                  <Text onPress={onResend} style={{ color: "#5E30C1" }}>
                    Resend
                  </Text>
                </Text>
              ...
              <TouchableOpacity
                ...
                style={{
                  ...
                  backgroundColor: pin.length === 4 ? "#333333" : "#726E70",
                }}
              >
                {!loading && (
                  <Image
