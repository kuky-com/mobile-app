...
const DislikeSelectScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    analytics().logScreenView({
      screen_name: "DislikeSelectScreen",
      screen_class: "DislikeSelectScreen",
    })
  }, [])
  ...
  const onSave = () => {
    ...
    apiClient
      .post("interests/update-dislikes", { dislikes: tagsNames })
      .then((res) => {
        setLoading(false);
        if (res && res.data && res.data.success) {
          if (res.data.data && res.data.data.length < tagsNames.length) {
            ...
            showAlert("", `Oops! Some words doesn't look like an English word. Please try again.`, [
              { text: "Ok" },
            ]);
          } else {
            if (onUpdated) {
...
export default DislikeSelectScreen;
