...
const InterestVideoScreen = ({ navigation, route }) => {
    ...
    useEffect(() => {
        analytics().logScreenView({
            screen_name: 'InterestVideoScreen',
            screen_class: 'InterestVideoScreen'
        })
    }, [])
...
- Do not return empty string as an object inside arrray`
    ...
    const updateProfile = (video, audio) => {
        ...
        apiClient.post('users/update', { video_interests: video, audio_interests: audio, is_video_interests_blur: isBlur })
            .then((res) => {
                ...
                if (res && res.data && res.data.success) {
                    ...
                    NavigationService.reset('MatchingInfoUpdateScreen', { canClose: true })
    ...
    const processAudio = async (processingData) => {
        try {
    ...
    const uploadFileToAws = async () => {
        try {
            ...
            const res = await axios.post('https://ugfgxk4hudtff26aeled4u3h3u0buuhr.lambda-url.ap-southeast-1.on.aws', {
                s3_uri: `s3://kuky-video/public/${audioFileName}`
            })

            if (res && res.data && res.data.tags) {
                await processAudio(res.data)
            } else {
                showAlert(images.video_error, 'Oops!', 'Something went wrong while processing your video.', 'Please try again recording your video', [
                    { text: 'Record Again', onPress: () => navigation.goBack() }
                ])
        ...
        <View style={{ flex: 1, width: "100%" }}>
            ...
            <View style={{ flex: 1, width: "100%", alignItems: "center", justifyContent: "center" }}>
                ...
                <View
                    style={{
                        width: "100%",
                        height: Dimensions.get("screen").height - insets.bottom - insets.top - 360,
                        alignItems: "center",
                        justifyContent: "center",
                        flexDirection: "row",
                    }}
                ...
                >
                    ...
                    {!videoUrl && !recording &&
                        ...
                        >
                            <Text style={{ fontSize: 16, fontWeight: "700", color: "#333333" }}>
                                {"Add my interest manually instead"}
                            </Text>
                        </TouchableOpacity>
                    }
                    {recording && !videoUrl && (
            ...
            {((permission && !permission.granted) || (audioPermission && !audioPermission.granted)) && (
                ...
                >
                    ...
                    >
                        ...
                        >{`To continue, Kuky needs access to your camera. \nThis is required for recording your video interesting/dislikes.`}</Text>
                        <Text
                            style={{
                                lineHeight: 21,
                                fontSize: 14,
...
export default InterestVideoScreen;
