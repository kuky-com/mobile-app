...
const SpecificJPFScreen = ({ navigation, route }) => {
    ...
    const getCategoryList = async () => {
        try {
            setFetching(true)
            const res = await apiClient.get(`journeys/jpf-questions?journey_id=${currentUser.journey_id}`)
    ...
    return (
        <View style={[styles.container, { paddingTop: insets.top + 16, paddingBottom: insets.bottom + 8 }]}>
            ...
            <Text style={{ fontSize: 18, fontWeight: 'bold', color: 'white' }}>{`Let’s Dive Deeper`}</Text>
...
export default SpecificJPFScreen;
