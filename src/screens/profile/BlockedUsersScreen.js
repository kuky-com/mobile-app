...
const BlockedUsersScreen = ({ navigation }) => {
  ...
  const unBlockUser = (user, index) => {
    apiClient
      .post("users/unblock-user", { friend_id: user.blocked_id })
      .then((res) => {
        if (res && res.data && res.data.success) {
    ...
    return (
      <View
        ...
        style={{
          ...
          paddingVertical: 10,
          borderBottomWidth: 1,
          borderBottomColor: "#78787988",
        }}
      >
        {/* <Image source={{ uri: item?.blockedUser?.avatar }} style={{ width: 60, height: 60, borderRadius: 30 }} /> */}
        <AvatarImage
          avatar={item?.blockedUser?.avatar}
          full_name={item?.blockedUser?.full_name}
          style={{ width: 60, height: 60, borderRadius: 30 }}
        />
        <Text style={{ flex: 1, fontSize: 16, color: "black", fontWeight: "bold" }}>
          {item?.blockedUser?.full_name}
        </Text>
        <TouchableOpacity
          onPress={() => unBlockUser(item, index)}
          style={{
            paddingHorizontal: 5,
            paddingVertical: 5,
            alignItems: "center",
