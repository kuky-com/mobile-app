...
const GetMatchScreen = ({ navigation, route }) => {
  ...
  const onContinue = () => {
    if (currentUser?.id === match.sender_id) {
      NavigationService.replace("MessageScreen", {
        conversation: { ...match, profile: match.receiver },
      });
    } else {
      NavigationService.replace("MessageScreen", {
        conversation: { ...match, profile: match.sender },
      });
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text
        style={{ fontSize: 26, fontWeight: "bold", color: "#E8FF58" }}
      >{`You've Connected!`}</Text>
      <Text style={{ fontSize: 20, fontWeight: "bold", color: "white", textAlign: "center" }}>
        {`You’ve found someone to share your journey with.`}
      </Text>
      <ImageBackground
        source={images.wave_bg}
        style={{
